<x-layout bodyClass="g-sidenav-show  bg-gray-200">
    ...
    <x-navbars.sidebar activePage="create-client"></x-navbars.sidebar>
                    ...
                    <div class="card my-4">
                        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                            <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                                <h6 class="text-white text-capitalize ps-3">
                                    {{ isset($client) ? 'Edit' : 'Add new' }} Clients
                                </h6>
                            </div>
                        </div>
                        <div class="card-body p-3">
                            ...
                            <form
                                ...
                                @csrf
                                <div class="row">
                                    <input type="hidden" name="id" value="{{isset($client->id)? $client->id: 'new'}}"/>
                                        ...
                                        <div class="mb-3 {{$attribute != 'description'? 'col-md-6': 'col-md-12' }}">
                                            ...
                                            @if (str_contains($attribute, 'date'))
                                                <input type="date" class="form-control border border-2 p-2"
                                                       ...
                                                       value="{{ isset($client) ? date_format($client->$attribute, 'Y-m-d') : old($attribute) }}"
                                                       required>
                                            @elseif ($attribute === 'description')
                                                <textarea class="form-control border border-2 p-2" rows="6"
                                                          id="{{ $attribute }}"
                                                          name="{{ $attribute }}">{{ isset($client) ? $client->$attribute : old($attribute) }}</textarea>
                                            @elseif ($attribute === 'status')
                                                <select class="form-control border border-2 p-2" id="{{ $attribute }}"
                                                        name="{{ $attribute }}" required>
                                                    <option
                                                        value="ongoing" {{ (isset($client) && $client->status === 'ongoing') ? 'selected' : '' }}>
                                                        Ongoing
                                                    </option>
                                                    <option
                                                        value="completed" {{ (isset($client) && $client->status === 'completed') ? 'selected' : '' }}>
                                                        Completed
                                                    </option>
                                                    <option
                                                        value="pending" {{ (isset($client) && $client->status === 'pending') ? 'selected' : '' }}>
                                                        Pending
                                                    </option>
                                                </select>
                                            @elseif(in_array($attribute , ['client_id', 'budget', 'progress']))
                                                <input class="form-control border border-2 p-2"
                                                       ...
                                                       name="{{ $attribute }}"
                                                       value="{{ isset($client) ? $client->$attribute : old($attribute) }}"
                                                       required>
                                            @elseif($attribute === 'published')
                                                ...
                                                <div class="form-check form-switch">
                                                    ...
                                                    <input id="{{ $attribute }}" name="{{ $attribute }}"
                                                           ...
                                                           value="1" {{ $client->$attribute ? 'checked' : '' }}
                                                           role="switch" {{isset($client->$attribute) && $client->$attribute? 'checked': ''}}>
                                                </div>
                                            @else
                            ...
                            @if(isset($client))
                                ...
                                <div class="mb-3 col-md-6">
                                    ...
                                    <x-subcomponent.fileupload-button model_related_to="client"
                                                                      model_id="{{$client->id}}">
